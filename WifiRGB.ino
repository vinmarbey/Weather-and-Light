...
#include "web_wetterstation.h"
#include "web_startseite.h"
...
const char* password = "Beyer_WG";
const char* deviceName = "wifi-rgb";
#endif
#ifdef nmb
const char* ssid = "Tech_D0050221";
const char* password = "DUHGHHVB";
const char* deviceName = "wifi-rgb";
#endif
...
IPAddress clientIP(192, 168, 0, 250); //the IP of the device
IPAddress gateway(192, 168, 0, 1); //the gateway
...
void setup(void) {
  ...
  server.on("/weather", HTTP_GET, []() {
    server.send_P(200, "text/html", WETTERSTATION);
  });
...
void handleRoot() {
  server.send_P(200, "text/html", STARTSEITE);
